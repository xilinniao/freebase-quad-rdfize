...
public class DemoFreebaseRdfizer {
	...
	public static void main (String []args) throws Exception {
		...
		if (args.length < 1 || args.length % 2 != 0) {
			throw new IllegalArgumentException("Usage: DemoFreebaseRdfizer list{<tsv file> <converted rdf file name>}");
		}
		
		for (int i = 0; i < args.length; i++) {
			
			BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(new File(args [i]))));
			//String src = args[i];
			i++;
			//String dest = args [i];
			//System.out.println (src + "--->" + dest);
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File (args[i]))));
			String assertion = null;
			while ((assertion = reader.readLine()) != null) {
				try {
					//display (rdfizer.transformData(assertion));
					
					List<StringBuffer> triples = rdfizer.transformData(assertion);
