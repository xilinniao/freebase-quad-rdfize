...
public class MQLFreebaseSchemaFetcher implements FbOntologyFetch {
	...
	private void handleResponseStatus(final JSON response) throws Exception {
		...
		if (!response.get(FbSchemaGlobals.CODE_FIELD).string()
				.equals(FbSchemaGlobals.CODE_SUCCESS_VALUE)) {
			throw new Exception(response.get(FbSchemaGlobals.CODE_FIELD)
					.string());
		}
	}

	// TBD : change the logic to be recursive, good for now as we have only 84
	// domain.
	@Override
	public void getDomain(List<CharSequence> domainList) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			domainList.add(domainResponse.get(i).get("id").string()
					.replace("/", ""));
		}
	}

	private JSON executeTypeQuery(String domain, JSON envelope) {
		JSON query = a(o("type", "/type/type", "domain", o("id", domain), "id",
				null, "limit", FbSchemaGlobals.DEFAULT_LIMIT));
	...
	private void fetchType(final JSON response, List<CharSequence> typeList)
		...
		for (int i = 0; i < size; i++) {
			String type = response.get(FbSchemaGlobals.RESULT_FIELD).get(i)
					.get("id").string();
	...
	public void getType(final String domain, final Object env,
		...
		if (cursor.type().equals(Type.BOOLEAN)) {
			...
			getType(domain,
					createEnvelope(FbSchemaGlobals.CURSOR_FIELD,
							response.get(FbSchemaGlobals.CURSOR_FIELD).bool()),
					typeList);
		} else {
			JSON response = executeTypeQuery(domain, envelope);
			fetchType(response, typeList);
			getType(domain,
					createEnvelope(FbSchemaGlobals.CURSOR_FIELD,
							response.get(FbSchemaGlobals.CURSOR_FIELD).string()),
					typeList);
		}
	}
	...
	private JSON executePropertyQuery(String domain, String type, JSON envelope) {
		JSON query = a(o(
				...
				domain,
				"types",
				o("id", type, "properties",
						a(o("id", null, "expected_type", a()))), "limit",
	...
	private void fetchProperties(final JSON response,
			List<Map<CharSequence, List<CharSequence>>> propList)
			throws Exception {
		if (propList == null)
			throw new NullPointerException("TypeList cannot be null");
		...
		int size = response.get("result").array().size();
		for (int i = 0; i < size; i++) {
			JSON propRsp = response.get("result").get(i).get("types")
					.get("properties");
			int propSize = propRsp.array().size();
			HashMap<CharSequence, List<CharSequence>> propMap = new HashMap<CharSequence, List<CharSequence>>();
			for (int j = 0; j < propSize; j++) {
				int expSize = propRsp.get(j).get("expected_type").array()
						.size();
				ArrayList<CharSequence> expTypeList = new ArrayList<CharSequence>();
				for (int k = 0; k < expSize; k++) {
					String type = propRsp.get(j).get("expected_type").get(k)
							.string();
	...
	public static void main(String[] args) throws Exception {
		...
		for (CharSequence domain : domains) {
			...
			fetcher.getType(fetcher.formatNames((String) domain),
					fetcher.createEnvelope(FbSchemaGlobals.CURSOR_FIELD, true),
					sequences);
			for (CharSequence sequence : sequences) {
				System.out.println("=============== Type: " + sequence);
				ArrayList<Map<CharSequence, List<CharSequence>>> props = new ArrayList<Map<CharSequence, List<CharSequence>>>();
